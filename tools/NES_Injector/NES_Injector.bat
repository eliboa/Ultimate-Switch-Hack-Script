...
chcp 65001 >nul
IF "%ushs_launch%"=="Y" (
	echo Ce script va vous permettre de préparer l'émulateur Nes Classic Edition avec vos jeux et vos images.
	...
	IF NOT EXIST "%source_roms%\*.*" (
		MD "%source_roms%"
		echo Attention, le dossier de roms "%source_roms%" n'existait pas au lancement du script, celui-ci ne peut continuer en l'état mais le dossier vient d'être créé pour que vous puissiez y placer vos roms au format "nes".
		goto:end_script
	)
...
"%script_base%\nconvert.exe" -out png -rmeta -resize 140 204 "%target_path%\images\%jpg%"
set box=%jpg:~0,-4%
set "box1=%box:&=%"
set box1=%box1:'=%
set box1=%box1:,=%
